...
struct Tree {
    ...
    void postorder(Node*);
    int Node_Count(Node *);
    int leaf_Count(Node *);// private function untuk menghitung leaf
};
...
// fungsi yang akan di call di main
...
// fungsi menghitung leaf dengan cara rekursif
int Tree::leaf_Count(Node * TreeNode){
    if (TreeNode == nullptr) return 0;
    // jika tidak punya child left & child right berarti dia node leaf
